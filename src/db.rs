use chrono::prelude::*;
use rusqlite::{Connection, types::Type,params};
use color_eyre::Result;
use crate::internal_encoding::{decode_v1, decode_v0, encode_v1};
...
impl Db {
    pub fn open() -> Result<Db> {
        ...
        let user_version: u32 = connection.query_row("SELECT user_version FROM pragma_user_version",[], |r| {
            r.get(0)
        })?;
    ...
    pub fn load(&self, name: &str) -> Result<Option<AudioClip>> {
        let mut stmt = self.0.prepare("SELECT id, name, date, sample_rate, opus FROM clips WHERE name = ?1")?;
    ...
    pub fn list(&self) -> Result<Vec<ClipMeta>> {
        let mut stmt = self.0.prepare("SELECT id, name, date FROM clips ORDER BY date")?;
        let clip_iter = stmt.query_map([], |row| {
            let date: String = row.get(2)?;
